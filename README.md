...
- `plugins/codex/install/agents/*.toml`
  - Codex 用 custom agent の同梱インストール素材です。Codex plugin の有効化後、必要に応じて personal custom agents へコピーして使います。
...
Custom agent 定義は Codex plugin だけでは自動登録されないため、配布物に含める install 素材として `plugins/codex/install/agents/` に置いています。使う環境では、plugin checkout または plugin install 先から personal custom agents へコピーします。
...
コピー後は、Codex の subagent workflow から `implementer`、`senior-implementer`、`responsibility-boundary-reviewer`、`refactor-patch-agent` を明示的に使えます。
